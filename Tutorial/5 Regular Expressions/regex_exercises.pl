...
#1
#$_ = "#hee hoo haha";
#print "It matches mate\n" if $_ !~ /^#/;
#/(\w+)$/;
#$_ !~ /^#/;
#s/#{2,}/#/g;

# use contents of gettysburg.txt file
my $gettysburg = <<'END';
...
#my $word = 'we';
#my @count = $gettysburg =~ /\b$word\b/gi;
#print scalar @count, "\n";
#s/Foes/Flee/g;

$gettysburg =~ s/\n/,/g;
#print $gettysburg, "\n";
my @split_text = split /\./,$gettysburg;
#print @split_text, "\n";

my @new_joined_text = join ".\n\n\t", @split_text;
#print @new_joined_text;


#4 bubble sort
my @letters = ("a", "j", "k", "l", "q", "e", "z", "m");

for my $i ( 1 .. @letters ) {
    for my $k ( 0 .. $i - 1 ) {
        @letters[ $k, $k + 1 ] = @letters[ $k + 1, $k ]
            if $letters[$k] gt $letters[ $k + 1 ];
    }
}

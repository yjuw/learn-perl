...
while (1) {
    ...
    for my $i (reverse (0..7)) { # Row
        ...
        my $chosen_piece = $chessboard[$starty]->[$startx];
        if (($chosen_piece eq "WK") or ($chosen_piece eq "BK")){
            # print(abs($startx-$endx)**2);
            # print "\n";
            # print(abs($starty-$endy)**2);
            # print "\n";
            # print(sqrt(13));
            # print "\n", sqrt(abs($startx-$endx)**2 + abs($starty-$endy)**2);  

            last unless (sqrt( abs($startx-$endx)**2 + abs($starty-$endy**2))) == sqrt(5);
        }
